...
\title{Learning Molecular Fingerprints through Convolutional Networks on Graphs}
...
%Predicting properties of molecules is usually done by first representing the molecular connectivity graph as a 
Molecular graphs are usually processed using hash-based functions to produce fixed-size fingerprint vectors, which are then fed into standard machine learning methods.
...
Given a fingerprint length $L$, and $F$ features at each layer, the parameters of neural fingerprints consist of a separate output weight matrix of size $F \times L$ for each layer, as well as a set of hidden-to-hidden weight matrices of size $F \times F$ at each layer, one for each possible degree (up to 5).
...
\For {$L = 1$ to $R$} \Comment {for each layer}
    \For {each atom $a$ in molecule}
		...
		\State $\vr_a \leftarrow {\color{myfavblue} \sigma(H_L^N \times \vv)}$ \Comment {{\color{myfavblue} smooth function}}
...
\raisebox{1.5mm}{\includegraphics[width=0.5\textwidth]{../../DeepMoleculesData/experiments/2015-05-25-delaney/43-random-vs-depth/figures/all_rmsestest_rmse.pdf}}
 %}
\caption{\emph{Left:} Comparison of pairwise distances between molecules, measured using circular fingerprints and neural fingerprints with large random weights.
...
The performance of neural fingerprints with large random weights closely matches the performance of circular fingerprints.}
...
%
\newcommand{\molfeature}[3]{\includegraphics[width=#3, clip, trim = 2mm 3mm 2mm 3mm]{../../DeepMoleculesData/experiments/2015-05-30-visualize-fps/1/figures/fp_#1_highlight_#2.pdf}}%
\begin{figure}[h]%
\vspace{-3em}
...
These fingerprints were trained as inputs to a linear model predicting solubility as measured in \citep{delaney_data_2004}.
Shown are the two fingerprint features with the strongest predictive weights.
The first fingerprint has a positive predictive relationship with solubility, and is most activated by fragments containing a hydrophilic R-OH group, a standard indicator of solubility.
...
We trained the same model architecture to predict toxicity as measured in \citet{tox21}.
...
Code for computing neural fingerprints is available at \url{github.com/HIPS/deepmolecules}.
...
\bibliographystyle{plain}%include/icml2015}
\end{document}