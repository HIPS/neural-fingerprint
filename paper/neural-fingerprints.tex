...
We also experimented with dropconnect [cite] (a variant of dropout in which weights are randomly set to zero instead of hidden units), but since it was unhelpful in practice (on the validation set error) we disabled it. 

We optimized using minibatches of size 100 using the Adam algorithm~\citep{Adam14}, a variant of RMSprop that includes momentum.
...
Dataset               & \cite{delaney_data_2004} &  ?             &  Tripod [cite]  &  Clean energy project [cite] \\
Dataset size          &    1129        &    ?           &  ?              &    ?  \\
